(function () {
  ...
  function resetAppState() {
    ...
    el.startDate.value = '';
    el.endDate.value = '';
    el.analysisSummary.innerHTML = 'Jalankan analisis untuk melihat hasil ringkas fitting harmonik.';
    ...
    setStatus('Belum ada file yang dipilih.', 'muted');
    clearPlots();
  }
  ...
  function loadDatasetIntoUI(dataset) {
    ...
    el.downloadReportBtn.disabled = true;
    el.downloadDataBtn.disabled = true;
    el.analysisSummary.innerHTML = 'File berhasil dibaca. Silakan atur sensor, rentang waktu, dan resolusi lalu klik <strong>Proses Data</strong>.';
    el.analysisSummary.className = 'analysis-summary';

    setStatus(`
      <div class="kv">
        <strong>File</strong><span>${dataset.fileName}</span>
  ...
  function buildPlotLayout(title, yTitle) {
    return {
  ...
  function renderPlots(analysis) {
    ...
    ], buildPlotLayout('Observasi', 'Elevasi (m)'), { responsive: true });

    Plotly.newPlot(el.plotModel, [
      {
        x,
        y: analysis.model,
        mode: 'lines',
    ...
    ], buildPlotLayout('Model', 'Elevasi (m)'), { responsive: true });
    ...
    ], buildPlotLayout('Observasi vs Model', 'Elevasi (m)'), { responsive: true });
    ...
    Plotly.newPlot(el.plotResidual, [
      {
        x,
        y: analysis.residual,
        mode: 'lines',
    ...
    ], buildPlotLayout('Residual', 'Residual (m)'), { responsive: true });
  }
  ...
  function buildReportText(dataset, sensorKey, analysis, options) {
    ...
    lines.push(`File input                      : ${dataset.fileName}`);
  ...
  function validateOptions(dataset, options) {
    ...
    if (options.start < firstTime || options.end > lastTime) {
      throw new Error('Rentang waktu harus berada di dalam rentang data file.');
    }
  }
  ...
  function handleDownloadReport() {
    ...
    const sensorKey = options.sensorKey;
    const report = buildReportText(state.dataset, sensorKey, analysis, options);
    downloadTextFile(`report_${sensorKey}_${options.resolutionMinutes}menit.txt`, report);
  }
